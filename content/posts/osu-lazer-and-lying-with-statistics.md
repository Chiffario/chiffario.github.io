...
If you've ever seen the [changelog page on osu.ppy.sh](https://osu.ppy.sh/home/changelog), you can see the graphs that display the current user representation across different versions of the client. These values are public under the [`/changelog` endpoint](https://osu.ppy.sh/docs/index.html#changelog11), making tracking these values basically trivial. As I've learned later on, they are also pushed only once every five minutes (for lazer, stable is more complicated), so I knew I can avoid actively polling the endpoint for no reason.
...
{{< image src="/img/lazer/tweet.png" position="center" >}}
